...
See 'LinearKNeighborRegressionStockApp.py' for example usage.
...
# local
from sources import QuoteRecords
...
class Portfolio(object):
    ...
    def get_data(self, symbols):
        ...
        df = QuoteRecords.get_data('^IXIC', self.fresh, self.start_date, self.end_date, self.days)
        df = df.rename(columns={'Adj Close': '^IXIC'}).drop('Volume', axis=1).dropna()
        for symbol in symbols:
            try:
                df_temp = QuoteRecords.get_data(symbol, self.fresh, self.start_date, self.end_date, self.days)
